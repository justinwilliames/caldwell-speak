...
for _ in $(seq 1 15); do
  q=$(pulsar_curl -sf "$D/queue?limit=1")
...
U0=$(pulsar_curl -sf "$D/usage" | json characters_used); U0=${U0:-0}
echo "  baseline characters_used=$U0; speaking $LEN chars (forces a live fetch)"
SPEAK=$(pulsar_curl -sf -X POST -H 'Content-Type: application/json' \
...
PROMPT=0
for t in $(seq 1 8); do
  U=$(pulsar_curl -sf "$D/usage" | json characters_used); U=${U:-0}
  if [ "$U" -ge "$((U0 + LEN))" ]; then
...
for _ in $(seq 1 20); do
  if pulsar_curl -sf "$D/history?limit=50" | python3 -c "import sys,json;ids=[e['id'] for e in json.load(sys.stdin)];sys.exit(0 if '$ID' in ids else 1)"; then
    FOUND=1; break
  fi
  sleep 1
done
...
REPLAY=$(pulsar_curl -sf -X POST -H 'Content-Type: application/json' -d "{\"id\":\"$ID\"}" "$D/history/replay")
RC=$?
...
CODE=$(pulsar_curl -s -o /dev/null -w '%{http_code}' -X POST -H 'Content-Type: application/json' \
